...
class InfiniteLevel(Scene):
    ...
    def __init__(self, random_pos: bool) -> None:
        ...
        if type(self).instance is None:
            self.maze = Maze.generate(
                term.width // 5, term.height // 3, random_pos=random_pos
            )
            random = random_pos
    ...
    def next_frame(self, val: Keystroke) -> Union[str, int]:
        ...
        if self.instance.first_frame:
            ...
            if not self.instance.t1.is_alive() or self.player_in_boxes(
        ...
        for y in range(0, num_box_y):
            for x in range(0, num_box_x):
                ...
                while (
                    ...
                    or box_pos.x < 2
                    or box_pos.y < 2
                    or box_pos.x > len(self.maze.matrix[1]) - 2
                    or box_pos.y > len(self.maze.matrix) - 2
                    or self.maze.matrix[box_pos.y][box_pos.x] != AIR
